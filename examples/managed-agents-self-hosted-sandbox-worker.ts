...
import type { BetaManagedAgentsSessionEvent } from '@puku-ai/sdk/vendor/resources/beta/sessions/events';
...
const environmentKey = requireEnv('PUKU_ENVIRONMENT_KEY');
...
async function main() {
  ...
  try {
    ...
    //    `PUKU_WORK_ID` / `PUKU_ENVIRONMENT_ID` / `PUKU_SESSION_ID` /
    //    `PUKU_ENVIRONMENT_KEY` environment variables, so `environmentId` /
