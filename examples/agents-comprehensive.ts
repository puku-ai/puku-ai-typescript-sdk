...
async function main() {
  // Optional: puku-native runtime does NOT call the GitHub MCP server (it
  // resolves `mcp_servers` to an empty list), so a token isn't required to
  // exercise the gateway surface. Pass a real token via env to populate
  // `client.beta.vaults.credentials.create` with a real credential; without
  // it, the example still completes end-to-end on puku because the vault
  // stub accepts the credential create regardless.
  const githubToken = process.env['GITHUB_TOKEN'] ?? 'puku-stub-github-token';
  ...
  const skillContent = fs.readFileSync(path.join(import.meta.dir, 'greeting-SKILL.md'));
