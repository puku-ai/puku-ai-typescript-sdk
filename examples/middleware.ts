...
// clients it observes the same canonical request (`model` and
// `stream` in the body, `puku-beta` as a header) and normalized SSE
  ...
  const message = await client.messages.create(
    {
      model: 'puku-ai-2.8',
      max_tokens: 100,
      messages: [{ role: 'user', content: 'Hey Puku!?' }],
    },
    {
